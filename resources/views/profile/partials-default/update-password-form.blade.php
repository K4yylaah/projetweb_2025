<section>
    ...
    <form method="post" action="{{ route('password.update') }}" class="card-body flex flex-col gap-5 p-10">
        ...
        <div>
            <x-forms.input label="{{ __('Current Password') }}" name="current_password" type="password" :placeholder="__('Enter your current password')"
        ...
        <div>
            <x-forms.input label="{{ __('Confirm Password') }}" name="password_confirmation" type="password"
                :placeholder="__('Confirm new password')" :messages="$errors->updatePassword->get('password_confirmation')" />
        </div>
